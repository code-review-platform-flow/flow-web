...
export const emailAuth = async (email: string, universityName: string): Promise<EmailAuthResponse> => {
    try {
        const response: EmailAuthResponse = await ky
            .post(`${process.env.NEXT_PUBLIC_SERVER_URL}/auth/email`, {
...
export const codeAuth = async (email: string, universityName: string, code: number): Promise<CodeAuthResponse> => {
    try {
