...
export const fetchHallOfFameList = async (): Promise<HallOfFameUser[]> => {
    const response = await ky.get(`${process.env.NEXT_PUBLIC_SERVER_URL}/hall-of-fame/9`);
    if (!response.ok) {
...
export const fetchHallOfFameListDetail = async (): Promise<any[]> => {
    try {
        // Hall of Fame 사용자 목록을 가져옴
        const userList = await fetchHallOfFameList();
